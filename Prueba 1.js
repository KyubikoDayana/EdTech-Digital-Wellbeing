...
/* =========================================
     FUNCIONALIDAD BOTÓN NEXT DEL QUIZ
     ========================================= */
  ...
  /* REFUERZO DEL SISTEMA DE TRADUCCIÓN */
  const updateAllTranslations = (lang) => {
    document.querySelectorAll("[data-en]").forEach(element => {
      const translation = element.getAttribute(`data-${lang}`);
      if (translation) {
        // Si es un input o botón con texto directo
        element.textContent = translation;
      }
    });
  };

  // Modificar el evento de los botones de idioma existentes para ser más robustos
  langButtons.forEach(button => {
    button.addEventListener("click", () => {
      const selectedLang = button.textContent.toLowerCase();
      document.documentElement.lang = selectedLang;
      updateAllTranslations(selectedLang);
      updateQuiz(); // Actualiza también el feedback del quiz
    });
  });